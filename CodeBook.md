...
The full table includes 210 observation rows (beyond the one title row of the column names) and x variables. The total columns is 86 + 2 (variables + the subject code and activity name columns).
...
The domain for names with "t" as in "tBodyAcc-mean-X" is time, and the domain for names with "f" as in "fBodyAcc-mean-X" is frequency. Domain described instead of units since all variables were ormalized and bounded within [-1,1].
...
The full description of the workflow to clean up the data and creating the tidy data file **Mean_ActivityDataBySmartphones.txt**, was executed by the script [run_analysis.R](https://github.com/anat-kedem/TidyDataActivityRecognitionUsingSmartphones/run_analysis.R) as described in the [README.md](https://github.com/anat-kedem/TidyDataActivityRecognitionUsingSmartphones/README.md) file.
